...
type Video struct {
	...
	Roles        []*Role   `gorm:"many2many:video_roles" json:"role,inline"`                           //主演
	...
	Sample       []*Sample `gorm:"foreignkey:video_id;association_foreignkey:id" json:"sample,inline"` //样板图
...
func (v *Video) JSON() string {
	v.VideoID = v.ID
	//log.Infof("sample:%+v,tags:%+v", v.Sample, v.Tags)
