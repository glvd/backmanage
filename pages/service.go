...
import (
	...
	"github.com/glvd/themes/sword/components/card"
	"github.com/go-echarts/go-echarts/charts"
	"html/template"
	"math/rand"
)

// GetServiceDashBoardContent ...
func GetServiceDashBoardContent() (types.Panel, error) {
	...
	gauge.Width = "250px"
	gauge.Height = "250px"
	//ecboxCol7 := colComp.SetSize(map[string]string{"md": "9"}).SetContent(components.Box().WithHeadBorder().
	//	SetHeader("任务状态").SetBody().GetContent()).GetContent()
	infoboxCol1 := colComp.SetSize(size).SetContent(echart.SetContent(gauge).GetContent()).GetContent()
